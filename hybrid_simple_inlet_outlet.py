...
class Inlet(InletBase):
    def initialize(self):
        super().initialize()
        self.uinlet = self.inletinfo.umax
    ...
    def update(self, time, dt, stage):
        ...
        if stage in self.active_stages:
            ...
            inlet_pa.u[all_idx] = self.uinlet
    ...
    def add_io_properties(self, pa, scheme=None):
        ...
        for const in consts:
            if const not in pa.constants:
                pa.add_constant(const, 0.0)

    def get_stepper(self, scheme, cls, edactvf=True):
        from pysph.sph.bc.inlet_outlet_manager import OutletStepWithUhat
        steppers = {}
        if cls == EPECIntegrator:
...
class SimpleInletOutlet1(InletOutletManager):
    def add_io_properties(self, pa, scheme=None):
        N = 6
        DEFAULT_PROPS = [
    ...
    def get_stepper(self, scheme, cls, edactvf=True):
        ...
        steppers = {}
        if isinstance(scheme, AdaptiveEDACScheme):
            for inlet in self.inlets:
                steppers[inlet] = InletStep()
            for outlet in self.outlets:
                steppers[outlet] = OutletStepWithUhat()
    ...
    def get_equations(self, scheme=None, summation_density=False,
        ...
        for name in self.ghost_inlets:
            ...
            g04.append(EvaluatePropertyfromCharacteristicsInlet(dest=name, sources=None))
        for name in self.outlets:
...
class EvalauteCharacterisctics(Equation):
    ...
    def initialize(self, d_idx, d_u, d_p, d_J1, d_J2u, d_uta, d_pta, d_J3p):
        rho0 = self.rho0
        cs = self.c0
        uref = d_uta[d_idx]
        pref = d_pta[d_idx]
        ...
        d_J2u[d_idx] = rho0 * cs * (d_u[d_idx] - uref) + (d_p[d_idx] - pref)
        d_J3p[d_idx] = -rho0 * cs * (d_u[d_idx] - uref) + (d_p[d_idx] - pref)
...
class EvaluatePropertyfromCharacteristicsInlet(Equation):
    def initialize(self, d_idx, d_pta, d_p, d_J3p, dt, t):
        if t > 20 * dt:
            j3p = d_J3p[d_idx]
            d_p[d_idx] = d_pta[d_idx] + 0.5 * (j3p)
